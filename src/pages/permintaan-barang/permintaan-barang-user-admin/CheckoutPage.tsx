...
import {
  customStylesInputWithoutRounded,
  formatDatePayload,
  formatNumber,
} from "@/utils/utils";
...
const CheckoutPage = () => {
  ...
  const status = "active";
  const dataListUserFiltered = useMemo(() => {
    return dataListUser?.filter((item: any) => item.status === status);
  }, [dataListUser]);

  const userOptions = dataListUserFiltered?.map((item) => ({
    value: item.id,
    label: item.fullname,
  }));
  ...
  const onProsesPesanan = async () => {
    ...
    try {
      const res: any = await axiosServices().post(
        "/api/transaksi/add-permintaan-barang",
        {
          data: cartItem.map((item) => ({
            ...
            nama_user: selectedUser.label,
            id_ruang_kerja: findNamaUser(selectedUser.value).id_ruang_kerja,
            nama_ruang_kerja: findNamaUser(selectedUser.value).nama_ruang_kerja,
      ...
      if (res.status === 200) {
        ...
        navigate("/list-permintaan-barang-user-admin");
      }
    } catch (error: any) {
      toast({
        title: "Gagal",
  ...
  if (!cartItem || loadingListUser) return <h1>Loading...</h1>;

  return (
    <main className="base-card">
      <TypoH1 className="mb-4">Konfirmasi Barang Admin</TypoH1>
