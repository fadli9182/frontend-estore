...
const ModalEditSuplier = ({ refetch, data, open, setOpen }: ModalEditProps) => {
  ...
  useEffect(() => {
    form.setValue("nama_supplier", data.nama_supplier);
    form.setValue("no_telp_supplier", data.no_telp_supplier);
    form.setValue("alamat_supplier", data.alamat_supplier);
  ...
  async function onSubmitEdit(values) {
    ...
    try {
      const res = await axiosServices().put("/api/supplier/update-supplier", {
        ...values,
      ...
      if (res.status === 200) {
        toast({
          title: "Berhasil",
          description: "Suplier berhasil diubah",
        });
        refetch();
        setOpen(false);
      }
